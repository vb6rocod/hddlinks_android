...
$has_add="yes";
$has_fs="yes";
$ref="https://coronamovies.net";
$host=parse_url($ref)['host'];
$fav_target="coronamovies_f_fav.php?host=".$ref;
$add_target="coronamovies_f_add.php";
$add_file="";
$fs_target="coronamovies_fs.php";
$target="coronamovies_f.php";
/* ==================================================== */
...
$r=array();
if ($tip=="release") {
  $videos = explode('div class="card border-1',$html);
  unset($videos[0]);
  $videos = array_values($videos);
  foreach($videos as $video) {
   if (preg_match("/href=\"([^\"]+)\"/",$video,$m)) {
    $link=$m[1];
    $t1=explode('text-truncate">',$video);
    $t2=explode('<',$t1[1]);
    $title=$t2[0];
  ...
  $videos = explode('div class="col-4"',$html);
  unset($videos[0]);
  $videos = array_values($videos);
  foreach($videos as $video) {
   if (preg_match("/href=\"([^\"]+)\"/",$video,$m)) {
