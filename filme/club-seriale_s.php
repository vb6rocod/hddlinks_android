...
$link=$_GET["link"];
$width="200px";
$height=round((299/273)*200)."px";
/* ==================================================== */
$has_fav="no";
$has_search="no";
$has_add="no";
$has_fs="yes";
$fav_target="club-seriale_s_fav.php?host=https://www.club-seriale.online";
$add_target="club-seriale_s_add.php";
$add_file="";
$fs_target="club-seriale_s_ep.php";
$target="club-seriale_s.php";
/* ==================================================== */
...
if ($tip=="release") {
if($page>1) {
  $l ="https://www.club-seriale.online/seriale?page=".$page;
} else {
	$page = 1;
  $l="https://www.club-seriale.online/seriale";
}
} else {
  $l="https://filmeonlinegratis.org/?s=".str_replace(" ","+",$tit);
}
$r=array();
$ua = $_SERVER['HTTP_USER_AGENT'];
  ...
  $videos = explode('class="col-xs', $html);
  unset($videos[0]);
  $videos = array_values($videos);
  foreach($videos as $video) {
    $t1=explode('href="',$video);
    $t2=explode('"',$t1[1]);
    $link="https://www.club-seriale.online".$t2[0];
    ...
    $t2=explode('"',$t1[1]);
    $i=$t2[0];
    $image="https://www.club-seriale.online".$t2[0];
    if (strpos($link,"/seriale") !== false && $title && $i) array_push($r ,array($title,$link, $image));
  }
...
for ($k=0;$k<$c;$k++) {
  ...
  $title=trim(preg_replace("/(online subtitrat|serial).*/i","",$title));
  $link=$r[$k][1];
  $image=$r[$k][2];
  $rest = substr($title, -6);
