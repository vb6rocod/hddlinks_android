...
$has_add="yes";
$has_fs="yes";
$fav_target="streamlord_s_fav.php?host=http://www.streamlord.com";
$add_target="streamlord_s_add.php";
$add_file="";
$fs_target="streamlord_s_ep.php";
$target="streamlord_s.php";
/* ==================================================== */
...
if($tip=="release") {
  if ($page==1)
   $l="http://www.streamlord.com/tvshows.html";
  else
   $l="http://www.streamlord.com/series.php?page=".$page;
} else {
  $search=str_replace(" ","%20",$tit);
  $l="http://www.streamlord.com/searchtest.php";
  $post="searchapi2=".$search;
}
...
$r=array();
if ($tip=="release") {
  $videos = explode('<li id="sw"',$html);
  unset($videos[0]);
  $videos = array_values($videos);
  foreach($videos as $video) {
   $t1=explode('href="',$video);
   $t2=explode('"',$t1[1]);
   $link="http://www.streamlord.com/".$t2[0];
   $t3 = explode('movie-grid-title">', $video);
   $t4 = explode('<', $t3[1]);
   $title = trim($t4[0]);
   $title=prep_tit($title);
   $t1 = explode("src='", $video);
   $t2 = explode("'", $t1[1]);
   $image = "http://www.streamlord.com/".$t2[0];
   $t1=explode('movie-grid-year">',$video);
   $t2=explode('<',$t1[1]);
   $year=trim($t2[0]);
   if (strpos($link,"watch-tvshow") !== false) $r[]=array($link,$title,$image,$year);
  }
} else {
  ...
  foreach($videos as $video) {
   ...
   $t2 = explode('"', $t1[1]);
   $image = $t2[0];
   if (strpos($link,"/series") !== false) $r[]=array($link,$title,$image,$year);
  }
}
$c=count($r);
for ($k=0;$k<$c;$k++) {
