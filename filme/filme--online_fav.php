...
foreach($arr as $key => $value) {
    ...
    $link="filme--online_ep.php?tip=tv&title=".urlencode(fix_t($title11))."&link=".urlencode($link1)."&image=".$image."&sez=&ep=%ep_tit=";

  if ($n==0) echo '<TR>';
  $fav_link="mod=del&title=".urlencode(fix_t($title11))."&link=".$link1."&image=".urlencode($image);
  if ($tast == "NU") {
  echo '<td class="mp" align="center" width="25%"><a class="imdb" id="myLink'.($p*1).'" href="'.$link.'" target="_blank"><img src="r_m.php?file='.$image.'" width="160px" height="100px"><BR>'.unfix_t(urldecode($title11)).'</a>';
  echo '<a onclick="ajaxrequest('."'".$fav_link."'".')" style="cursor:pointer;">*</a></TD>';
  } else {
   $year="";
  $val_imdb="title=".urlencode(fix_t($title11))."&year=".$year."&imdb=";
  echo '<td class="mp" align="center" width="25%"><a class="imdb" id="myLink'.($p*1).'" href="'.$link.'" target="_blank"><img src="r_m.php?file='.$image.'" width="160px" height="100px"><BR>'.unfix_t(urldecode($title11)).'<input type="hidden" id="imdb_myLink'.($w*1).'" value="'.$val_imdb.'"><input type="hidden" id="fav_myLink'.($w*1).'" value="'.$fav_link.'"></a></TD>';
  $w++;
  }
  $p++;
  $n++;
