...
curl_setopt($ch, CURLOPT_REFERER, "https://api.hdv.fun");
...
for ($k=0;$k<count($p);$k++) {
 ...
 for ($n=0;$n<count($p[$k]['sub']);$n++) {
   if ($p[$k]['sub'][$n]['lg'] == 'romanian' || $p[$k]['sub'][$n]['lg'] == 'english') {
     $s[]=$p[$k]['sub'][$n]['sub_id'];
     $s_l[]=$p[$k]['sub'][$n]['lg'];
   }
 }
...
echo '<table border="1" width="100%"><TR>';
for ($z=0;$z<count($s);$z++) {
echo '<TD class="mp"><a onclick="'."savesub('".$s[$z]."')".'"'." style='cursor:pointer;'>".$s_l[$z].'</a></td>';
}
echo '</TR>';
echo '<table border="1" width="100%">';
echo '<TR>';
