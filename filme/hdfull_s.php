...
echo '<TR><TD class="cat" ><a id="fav" href="hdfull_s_fav.php" target="blank">Favorite</a></TD>
<TD class="form" colspan="3">';
//title=star&page=1&file=search
echo '<form action="hdfull_s.php" target="_blank">Cautare serial:';
...
$host=$r["host"];
 ...
 $post="q=".str_replace(" ","+",$title)."&limit=500&timestamp=1234567890&verifiedCheck=";
  $ch = curl_init($l);
  curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US; rv:1.9.1.2) Gecko/20090729 Firefox/3.5.2 GTB5');
  curl_setopt($ch,CURLOPT_REFERER,"https://hdfull.me/tv-shows/list");
  curl_setopt($ch, CURLOPT_FOLLOWLOCATION  ,1);
...
foreach($videos1 as $video1) {
  ...
  if ($tast == "NU") {
  if ($first)  {
  echo '<td align="center" width="25%"><a name="'.$ref.'" href="hdfull_s_ep.php?tip=serie&file='.$link1.'&title='.urlencode(fix_t($title11)).'&image='.$image.'" target="_blank">'.$title11.'</a> <a onclick="ajaxrequest('."'".$fav_link."'".')" style="cursor:pointer;">*</a></TD>';
  $first=false;
  } else
  echo '<td align="center" width="25%"><a href="hdfull_s_ep.php?tip=serie&file='.$link1.'&title='.urlencode(fix_t($title11)).'&image='.$image.'" target="_blank">'.$title11.'</a> <a onclick="ajaxrequest('."'".$fav_link."'".')" style="cursor:pointer;">*</a></TD>';
  } else {
  $year="";
  $val_imdb="title=".urlencode(fix_t($title11))."&year=".$year."&imdb=";
  if ($first)  {
  echo '<td align="center" width="25%"><a class="imdb" id="myLink'.($w*1).'" name="'.$ref.'" href="hdfull_s_ep.php?tip=serie&file='.$link1.'&title='.urlencode(fix_t($title11)).'&image='.$image.'" target="_blank">'.$title11.'<input type="hidden" id="imdb_myLink'.($w*1).'" value="'.$val_imdb.'"><input type="hidden" id="fav_myLink'.($w*1).'" value="'.$fav_link.'"></a></TD>';
  $first=false;
  } else
  echo '<td align="center" width="25%"><a class="imdb" id="myLink'.($w*1).'" href="hdfull_s_ep.php?tip=serie&file='.$link1.'&title='.urlencode(fix_t($title11)).'&image='.$image.'" target="_blank">'.$title11.'<input type="hidden" id="imdb_myLink'.($w*1).'" value="'.$val_imdb.'"><input type="hidden" id="fav_myLink'.($w*1).'" value="'.$fav_link.'"></a></TD>';
...
for ($k=0;$k<count($r);$k++) {
  ...
  $imdb=$r[$k]["imdb"];
  $image = $r[$k]["image"];
  if ($title11 && strpos($r[$k]["meta"],"show") !== false) {
  if ($n==0) echo '<TR>';
  $fav_link="mod=add&title=".urlencode(fix_t($title11))."&link=".$link1."&image=".urlencode($image);
  if ($tast == "NU")
  echo '<td class="mp" align="center" width="25%"><a class="imdb" href="hdfull_s_ep.php?tip=serie&file='.$link1.'&title='.urlencode(fix_t($title11)).'&image='.$image.'" target="_blank"><img src="'.$image.'" width="200px" height="278px"><BR>'.$title11.'</a> <a onclick="ajaxrequest('."'".$fav_link."'".')" style="cursor:pointer;">*</a></TD>';
  else {
  $year="";
  $val_imdb="title=".urlencode(fix_t($title11))."&year=".$year."&imdb=".$imdb;
  echo '<td class="mp" align="center" width="25%"><a class ="imdb" id="myLink'.($w*1).'" href="hdfull_s_ep.php?tip=serie&file='.$link1.'&title='.urlencode(fix_t($title11)).'&image='.$image.'" target="_blank"><img src="'.$image.'" width="200px" height="278px"><BR>'.$title11.'<input type="hidden" id="imdb_myLink'.($w*1).'" value="'.$val_imdb.'"><input type="hidden" id="fav_myLink'.($w*1).'" value="'.$fav_link.'"></a></TD>';
   $w++;
  }
  $n++;
  if ($n == 4) {
