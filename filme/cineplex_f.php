...
echo '<a class="nav" href="cineplex_f.php?page='.($page-1).'&gen='.$gen.'&token='.$token.'&tip='.$tip.'&title='.$title.'">&nbsp;&lt;&lt;&nbsp;</a> | <a class="nav" href="cineplex_f.php?page='.($page+1).'&gen='.$gen.'&token='.$token.'&tip='.$tip.'&title='.$title.'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
}else {
echo '<TD colspan="4" align="right"><a class="nav" href="cineplex_f.php?page='.($page+1).'&gen='.$gen.'&token='.$token.'&tip='.$tip.'&title='.$title.'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
}
}
...
$post="loadmovies=showData&page=1&abc=All&genres=&sortby=Recent&quality=All&type=movie&q=".str_replace("+","%20",urlencode($title))."&token=".$token;
...
if ($tip=="search") {
  ...
  if ($n==0) echo '<TR>';
  if (strpos($link_fs,"/movies") !== false) {
  if ($tast == "NU") {
  echo '<td class="mp" align="center" width="25%"><a class="imdb" id="myLink'.($p*1).'" href="'.$link_fs.'" target="_blank"><img src="'.$image.'" width="200px" height="280px"><BR>'.$title1.' ('.$year.')<input type="hidden" id="imdb_myLink'.($p*1).'" value="'.$val_imdb.'"></a> <a onclick="ajaxrequest('."'".$fav_link."'".')" style="cursor:pointer;">*</a></TD>';
  } else {
...
echo '<a class="nav" href="cineplex_f.php?page='.($page+1).'&gen='.$gen.'&token='.$token.'&tip='.$tip.'&title='.$title.'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
}
echo "</table>";
?>
<br></div></body>
