...
echo '<a class="nav" href="popcorn_s.php?page='.($page-1).'&file='.$file.'&title='.urlencode($title).'">&nbsp;&lt;&lt;&nbsp;</a> | <a class="nav" href="popcorn_s.php?page='.($page+1).'&file='.$file.'&title='.urlencode($title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
}else {
echo '<TD class="form" colspan="3">';
//title=star&page=1&file=search
echo '<form action="popcorn_s.php" target="_blank">Cautare serial:';
...
echo '<TD colspan="1" align="right"><a class="nav" href="popcorn_s.php?page='.($page+1).'&file='.$file.'&title='.urlencode($title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
}
}

//https://tv-v2.api-fetch.website/show/tt0374455
if ($tip=="search")
   $requestLink="https://tv-v2.api-fetch.website/shows/".$page."?sort=last%20added&order=-1&keywords=".str_replace(" ","+",$title);
else
   $requestLink = "https://tv-v2.api-fetch.website/shows/".$page;
/*
...
for ($w=0;$w <count($r); $w++) {
  ...
  if ($n==0) echo '<TR>';
  if ($tast == "NU")
  echo '<td class="mp" align="center" width="25%"><a class="imdb" href="popcorn_s_ep.php?tip=movie&imdb='.urlencode($imdb).'&title='.urlencode(fix_t($title11)).'&image='.$image.'" target="_blank"><img src="'.$image.'" width="200px" height="280px"><BR>'.$title11.' - ('.$year.')</a></TD>';
  else {
  $val_imdb="title=".$title11."&year=".$year."&imdb=".$imdb;
  echo '<td class="mp" align="center" width="25%"><a class ="imdb" id="myLink'.($w*1).'" href="popcorn_s_ep.php?tip=movie&imdb='.urlencode($imdb).'&title='.urlencode(fix_t($title11)).'&image='.$image.'" target="_blank"><input type="hidden" id="imdb_myLink'.($w*1).'" value="'.$val_imdb.'"><img src="'.$image.'" width="200px" height="280px"><BR>'.$title11.' - ('.$year.')</a></TD>';
  }
  $n++;
  if ($n == 4) {
  echo '</tr>';
...
echo '<a class="nav" href="popcorn_s.php?page='.($page+1).'&file='.$file.'&title='.urlencode($title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
//}
echo "</table>";
?>
