...
$r=array();
if ($tip=="search")
  $requestLink = "https://www.moviesjoy.net/search/".str_replace(" ","+",$tit)."/page-".$page.".html";
 else
$requestLink="https://www.moviesjoy.net/movie/filter/series/latest/all/all/all/all/all/page-".$page.".html";
...
foreach($videos as $video) {
  ...
  $t2 = explode('"', $t1[1]);
  $image = $t2[0];
    if (strpos($link1,"-season") !== false) array_push($r ,array($title11,$link1, $image));
  }
  //print_r ($r);
?>
<html><head>
...
echo '<TD class="cat" width="25%"><a id="fav" a href="moviesjoy_s_fav.php">Favorite</a></TD>
...
for ($k=0;$k<$c;$k++) {
  ...
  if ($n==0) echo '<TR>';
  $link="moviesjoy_s_ep.php?tip=tv&title=".urlencode(fix_t($title))."&link=".urlencode($link_film)."&image=".$image."&sez=&ep=%ep_tit=";
   if ($tast == "NU")
