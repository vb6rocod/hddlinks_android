...
$has_add="yes";
$has_fs="yes";
$fav_target="moviesjoy_s_fav.php?host=https://www.moviesjoy.net";
...
if($tip=="release") {
  $l="https://www.moviesjoy.net/movie/filter/series/latest/all/all/all/all/all/page-".$page.".html";
} else {
...
foreach($videos as $video) {
  ...
  $rest = substr(trim($t1[0]), -6);
  if (preg_match("/\((\d+)\)/",$rest,$m)) {
   $year=$m[1];
   $tit_imdb=trim(str_replace($m[0],"",trim($t1[0])));
  } else {
   $year="";
   $tit_imdb=trim($t1[0]);
  }
