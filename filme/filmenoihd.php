...
if ($tip=="release") {
  ...
  $l="http://www.filmenoihd.biz/page/".$page."/?s=".$search;
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL, $l);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
  curl_setopt($ch, CURLOPT_USERAGENT, $ua);
  ...
  foreach($videos as $video) {
    ...
    $t2=explode('"',$t1[1]);
    $image=$t2[0];
    if (strpos($link,"/filme") !== false && $mod=="da") array_push($r ,array($title,$link, $image));
  }
  } else {
...
echo '<TD class="nav" style="height:10px;text-align:right">';
if ($page > 1)
...
for ($k=0;$k<$c;$k++) {
  ...
  if ($n==0) echo '<TR>';
  if ($tast == "NU")
    echo '<td class="mp" width="25%"><a class="imdb" href="'.$link_f.'" target="blank"><img src="'.$image.'" width="'.$width.'" height="'.$height.'"><BR>'.$title.'</a></TD>';
  else {
