...
echo '<a class="nav" href="filme-seriale_f.php?page='.($page-1).'&file='.$file.'&title='.urlencode($title).'">&nbsp;&lt;&lt;&nbsp;</a> | <a class="nav" href="filme-seriale_f.php?page='.($page+1).'&file='.$file.'&title='.urlencode($title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
}else {
echo '<TD class="form" colspan="2">';
//title=star&page=1&file=search
echo '<form action="filme-seriale_f.php" target="_blank">Cautare film:';
...
echo '<TD class="nav" colspan="2" align="right"><a href="filme-seriale_f.php?page='.($page+1).'&file='.$file.'&title='.urlencode($title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
}
}
...
if ($tip=="search") {
  ...
  if ($page >1)
  $requestLink="https://filmeonlinegratis.org/filmeonline/page/".$page."/";
  else
  $requestLink="https://filmeonlinegratis.org/filmeonline/";
  $ch = curl_init($requestLink);
  curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US; rv:1.9.1.2) Gecko/20090729 Firefox/3.5.2 GTB5');
  curl_setopt($ch,CURLOPT_REFERER,"https://filmeonlinegratis.org");
  //curl_setopt ($ch, CURLOPT_POST, 1);
...
foreach($videos as $video) {
  ...
  $title11=trim(preg_replace("/(gratis|subtitrat|onlin|film|sbtitrat|dublat|\shd)(.*)/i","",$title11));
  $id1=$link1;
  ...
  $year="";
  //300px × 168px
  if($link1!="" && strpos($link1,"sezonul") === false) {
  if ($n==0) echo '<TR>';
  if ($tast == "NU")
...
echo '<a class="nav" href="filme-seriale_f.php?page='.($page+1).'&file='.$file.'&title='.urlencode($title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
}
echo "</table>";
?>
<br></div></body>
