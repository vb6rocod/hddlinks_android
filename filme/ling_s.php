...
$has_add="yes";
$has_fs="yes";
$fav_target="ling_s_fav.php?host=https://".$host;
$add_target="ling_s_add.php";
$add_file="";
$fs_target="ling_ep.php";
$target="ling_s.php";
/* ==================================================== */
...
if($tip=="release") {
  ...
  //curl_setopt($ch, CURLOPT_TLS13_CIPHERS, 'TLS_AES_256_GCM_SHA384:TLS_CHACHA20_POLY1305_SHA256:TLS_AES_128_GCM_SHA256:TLS_AES_128_CCM_8_SHA256:TLS_AES_128_CCM_SHA256');
  $html = curl_exec($ch);
...
foreach($videos as $video) {
  ...
  $t2 = explode('"', $t1[1]);
  $image = "https://".$host.trim($t2[0]);
  $t1=explode('p>',$video);
  if (preg_match("/\d{4}/",$t1[1],$m))
   $year=$m[0];
  else
   $year="";
  $imdb="";
