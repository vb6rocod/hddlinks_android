...
if($tip=="release") {
 if ($page>1)
  ...
  if ($page == 1)
    $l="https://".$host."/search?q=".$search."&type=movie";
  else
    $l="https://".$host."/search?q=".$search."&type=movie&page=".$page;
}
//https://filmehd.one/search?q=star&type=movie
...
$ua = $_SERVER['HTTP_USER_AGENT'];
  ...
  foreach($videos as $video) {
    ...
    $t1=explode('class="card__name"',$video);
    $t2=explode(">",$t1[1]);
    $t3=explode('<',$t2[1]);
    $title=trim($t3[0]);
