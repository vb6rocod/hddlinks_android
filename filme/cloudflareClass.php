<?php
class cloudflare {
public static function get_base_cookie() {
	...
	*/
	...
	private static function bypassCloudFlare($siteNetLoc) {	
		...
		if($cfAnswerParams = self::solveJavaScriptChallenge($siteNetLoc, $ddosPage['content'])) {
			...
			//https://www.moviesplanet.tv/cdn-cgi/l/chk_jschl?jschl_vc=6719aa04a358c5add3e51c94c2407850&pass=1507359524.229-9OtH46L3rw&jschl_answer=119
	...
	private static function getCookie($siteNetLoc) {
		...
		$cfSessionFile = self::get_base_cookie().md5($siteNetLoc).".dat";
	...
	*/
	private static function getPage($url, $referer, $headers = array()){
		...
		if($curlResource = curl_init($url)){
        		...
        		// session cookies
        		//curl_setopt($curlResource, CURLOPT_COOKIEJAR,  $cookie);
        		//curl_setopt($curlResource, CURLOPT_COOKIEFILE, $cookie);
        		// return settings
