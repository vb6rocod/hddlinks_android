...
function ajaxrequest(link) {
  ...
  var the_data = link;
  //alert(the_data);
  var php_file="flixtor_f_add.php";
...
echo '<a class="nav" href="flixtor_f.php?page='.($page-1).'&file='.$file.'&title='.urlencode($title).'">&nbsp;&lt;&lt;&nbsp;</a> | <a class="nav" href="flixtor_f.php?page='.($page+1).'&file='.$file.'&title='.urlencode($title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
}else {
echo '<TR><TD class="cat" ><a class "imdb" id="fav" href="flixtor_f_fav.php" target="blank">Favorite</a></TD>
<TD class="form" colspan="2">';
//echo '<TD colspan="2">';
//title=star&page=1&file=search
//https://flixtor.ac/?c=movie&m=more_movies
//page=3&position=0&keyword=&order_by=movies&yrf=&yrt=&rating=&votes=&lang=&genre=
echo '<form action="flixtor_f.php" target="_blank">Cautare film:';
...
echo '<TD colspan="1" align="right"><a class="nav" href="flixtor_f.php?page='.($page+1).'&file='.$file.'&title='.urlencode($title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
}
}

if ($tip=="search") {
  $requestLink = "https://flixtor.ac/?c=movie&m=more_movies";
  $post="page=".$page."&position=0&keyword=".str_replace(" ","%20",$title)."&order_by=&yrf=&yrt=";
} else {
  $requestLink = "https://flixtor.ac/?c=movie&m=more_movies";
  $post="page=".$page."&position=0&keyword=&order_by=movies&yrf=&yrt=&rating=&votes=&lang=&genre=";
}
...
'Cookie: approve=1; approve_search=yes'
  ...
  //curl_setopt($ch,CURLOPT_REFERER,"https://flixtor.ac");
...
//echo $html;
//die();
 $videos = explode('div class="cflip m-0">', $html);
unset($videos[0]);
...
foreach($videos as $video) {
  ...
  $t1 = explode('"',$t0[1]);
  $link1 = $t1[0];
  //if (strpos($link1,"http") === false) $link1="https://www1.subsmovies.nz/watch?movie=".$link1;
  $t1 = explode('card-text title sh1">', $video);
  $t2 = explode('<', $t1[1]);
  $title11 = $t2[0];
  ...
  $t2 = explode('"', $t1[1]);
  $image = $t2[0];
  $link2='flixtor_fs.php?tip=movie&link='.urlencode($link1).'&title='.urlencode(fix_t($title11)).'&image='.$image."&sez=&ep=&ep_tit=";
...
echo '<a class="nav" href="flixtor_f.php?page='.($page+1).'&file='.$file.'&title='.urlencode($title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
}
echo "</table>";
?>
<br></div></body>
