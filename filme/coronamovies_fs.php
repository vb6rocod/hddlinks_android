...
'Connection: keep-alive');
  ...
  $html = curl_exec($ch);
  curl_close ($ch);
  $t1=explode('subtitlesSource:[',$html);
  $t2=explode('],',$t1[1]);
  $s=json_decode("[".$t2[0]."]",1);
  $sub=array();
  for ($p=0;$p<count($s);$p++) {
   $sub[$s[$p]['subtileLabel']]=$s[$p]['subtitlePath'];
  }
  //print_r ($sub);
  $lang="";
  if (isset($sub['Romanian'])) {
    $srt="https:".$sub['Romanian'];
    $lang="Romana";
  } elseif (isset($sub['English'])) {
    $srt="https:".$sub['English'];
    $lang="Engleza";
  } else
    $srt="";
  if (preg_match_all("/setMp4Source\(\'([^\']+)\'/",$html,$m))
   $r=$m[1];
   //print_r ($m);
echo '<table border="1" width="100%">';
echo '<TR><TD class="mp">Alegeti un server: Server curent:<label id="server">'.parse_url($r[0])['host'].'</label>
<input type="hidden" id="file" value="'.urlencode("https://coronamovies.net?file=".$r[0]."&sub=".$srt).'"></td></TR></TABLE>';
echo '<table border="1" width="100%"><TR>';
$k=count($r);
$x=0;
for ($i=0;$i<$k;$i++) {
  if ($x==0) echo '<TR>';
  $c_link="https://coronamovies.net?file=".$r[$i]."&sub=".$srt;
