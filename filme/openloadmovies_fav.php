...
$file=$base_fav."openloadmovies.dat";
$arr=array();
$h="";
if (file_exists($file)) {
...
foreach($arr as $key => $value) {
    ...
    $image=$arr[$key]["image"];
    $image1=$image;
  $link='openloadmovies_fs.php?tip=movie&link='.$link1.'&title='.urlencode(fix_t($title11))."&image=&sez=&ep=&ep_tit=";

  if ($n==0) echo '<TR>';
  $fav_link="mod=del&title=".urlencode(fix_t($title11))."&link=".$link1."&image=".urlencode($image);
  if ($tast == "NU") {
  echo '<td class="mp" align="center" width="25%"><a class="imdb" id="myLink'.($p*1).'" href="'.$link.'" target="_blank"><img src="'.$image1.'" width="200px" height="280px"><BR>'.unfix_t(urldecode($title11)).'</a>';
  echo '<a onclick="ajaxrequest('."'".$fav_link."'".')" style="cursor:pointer;">*</a></TD>';
  } else {
  $year="";
  $val_imdb="title=".urlencode(fix_t($title11))."&year=".$year."&imdb=";
  echo '<td class="mp" align="center" width="25%"><a class ="imdb" id="myLink'.($p*1).'" href="'.$link.'" target="_blank"><img src="'.$image1.'" width="200px" height="280px"><BR>'.unfix_t(urldecode($title11)).'';
