...
if ($tip=="release") {
  $l="https://filmetop.org/seriale/page/".$page."/";
  $ch = curl_init();
  ...
  //echo $html;
  if ($tip=="release") {
  $videos = explode('<li class="TPostMv', $html);
  unset($videos[0]);
...
echo '<TD class="cat" width="25%"><a id="fav" a href="filmetop_fav.php">Favorite</a></TD>
...
for ($k=0;$k<$c;$k++) {
  ...
  if ($n==0) echo '<TR>';
  $link="filmetop_ep.php?tip=tv&title=".urlencode(fix_t($title))."&link=".urlencode($link_film)."&image=".$image."&sez=&ep=%ep_tit=";
   if ($tast == "NU")
