...
if ($tip=="movie") {
  ...
  $l="https://fmovies.co/ajax/film/servers/".$id."?ts=".$ts."&_=";
  //echo $l;
  $ch = curl_init();
  ...
  $x=json_decode($h,1);
  $y=$x['html'];
  $videos = explode('<div class="server row"', $y);
  unset($videos[0]);
  ...
  foreach($videos as $video) {
   ...
   $t3=explode('"',$t1[2]);
   $s[]="Server".$t2[0];
   $r[]="https://fmovies.co/ajax/episode/info?ts=".$ts."&_=694&id=".$t3[0]."&server=".$t2[0];
  }
} else {
 $r[]=$link;
}
