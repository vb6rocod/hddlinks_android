...
if ($tip=="release") {
  ...
  //echo $html;
  if ($tip=="release") {
  $videos = explode('class="home-box">', $html);
  unset($videos[0]);
  ...
  foreach($videos as $video) {
    ...
    $t2=explode('"',$t1[1]);
    $link=$t2[0];
    $t1=explode('title tags">',$video);
    $t2=explode("<",$t1[1]);
    $title=decode_entities($t2[0]);
    $t1=explode("src=",$video);
    $t2=explode('&',$t1[1]);
    $image=$t2[0];
    if (strpos($link,"/show") !== false) array_push($r ,array($title,$link, $image));
  }
  } else {
    $z=json_decode($html,1);
    //print_r ($z);
    for ($k=0;$k<count($z);$k++) {
     $link=$z[$k]["permalink"];
     $img1=$z[$k]["image"];
...
echo '<TD class="cat" width="25%"><a id="fav" a href="seriestop_s_fav.php">Favorite</a></TD>
...
for ($k=0;$k<$c;$k++) {
  ...
  if ($n==0) echo '<TR>';
  $link="seriestop_s_ep.php?tip=tv&title=".urlencode(fix_t($title))."&link=".urlencode($link_film)."&image=".$image."&sez=&ep=%ep_tit=";
   if ($tast == "NU")
