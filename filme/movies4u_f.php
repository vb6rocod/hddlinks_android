...
$has_add="yes";
$has_fs="yes";
$fav_target="movies4u_f_fav.php?host=".$last_good;
$add_target="movies4u_f_add.php";
$add_file="";
$fs_target="movies4u_fs.php";
$target="movies4u_f.php";
/* ==================================================== */
...
if ($tip=="search") {
 ...
 $l="https://www1.movie4u.live/wp-json/dooplay/search/?keyword=".$search."&nonce=b25f8f344b";
} else {
...
foreach($videos as $video) {
 ...
 $t3=explode('"',$t1[1]);
 $title=$t3[0];
  if ($title && strpos($tip,"featured") === false) $f[] = array($title,$link,$image);
}
} else {
 $x=json_decode($h,1);
 foreach ($x as $key=>$value) {
   ...
   $link=$x[$key]['url'];
   $image=$x[$key]['img'];
   if ($title && strpos($link,"/tvshows") === false) $f[] = array($title,$link,$image);
 }
}
//echo $html;
