...
if ($tip=="release") {
  ...
  //echo $html;
  if ($tip=="release") {
  $videos = explode("li class='category-item'", $html);
  unset($videos[0]);
  ...
  foreach($videos as $video) {
    ...
    $t2=explode('"',$t1[1]);
    $image=$t2[0];
    if (strpos($link,"/serie") !== false) array_push($r ,array($title,$link, $image));
  }
  } else {
  //echo $html;
  $videos = explode('div class="search-item clearfix">', $html);
  unset($videos[0]);
  $videos = array_values($videos);
  //print_r ($videos);
  foreach($videos as $video) {
    $t0=explode('div valign="top"',$video);
    $t1=explode('href="',$t0[1]);
    $t2=explode('"',$t1[1]);
    $link=$t2[0];
    $t3=explode('title="',$t1[1]);
    $t4=explode('"',$t3[1]);
    $title=decode_entities($t4[0]);
    $t1=explode('src="',$video);
    $t2=explode('"',$t1[1]);
    $image=$t2[0];
    if (strpos($link,"/serie") !== false) array_push($r ,array($title,$link, $image));
...
echo '<TD class="cat" width="25%"><a id="fav" a href="swatchseries_s_fav.php">Favorite</a></TD>
...
for ($k=0;$k<$c;$k++) {
  ...
  if ($n==0) echo '<TR>';
  $link="swatchseries_s_ep.php?tip=tv&title=".urlencode(fix_t($title))."&link=".urlencode($link_film)."&image=".$image."&sez=&ep=%ep_tit=";
   if ($tast == "NU")
