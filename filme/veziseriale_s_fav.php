...
echo '<TR><TD class="cat"><a href="veziseriale.php" target="_blank">Vezi toate...</TD></TR></TABLE>';
$file=$base_fav."veziseriale_s_new.dat";
$arr=array();
$h="";
if (file_exists($file)) {
...
foreach($arr as $key => $value) {
    ...
    $title11 = $key;
    $image=$arr[$key]["image"];
    $link='veziseriale_s_ep.php?tip=serie&file='.$link1.'&title='.urlencode(fix_t($title11)).'&image='.urlencode($image);
  if ($n==0) echo '<TR>';
  $fav_link="mod=del&title=".urlencode(fix_t($title11))."&link=".$link1."&image=".urlencode($image);
  if ($tast == "NU") {
  echo '<td class="mp" align="center" width="25%"><a class="imdb" id="myLink'.($p*1).'" href="'.$link.'" target="_blank"><img src="r_m.php?file='.$image.'" width="200px" height="100px"><BR>'.unfix_t(urldecode($title11)).'</a>';
  echo '<a onclick="ajaxrequest('."'".$fav_link."'".')" style="cursor:pointer;">*</a></TD>';
  } else {
   $year="";
  $val_imdb="title=".urlencode(fix_t($title11))."&year=".$year."&imdb=";
  echo '<td class="mp" align="center" width="25%"><a class="imdb" id="myLink'.($p*1).'" href="'.$link.'" target="_blank"><img src="r_m.php?file='.$image.'" width="200px" height="100px"><BR>'.unfix_t(urldecode($title11)).'';
