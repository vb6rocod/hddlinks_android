...
$link_f=$fs_target.'?tip=series&link='.urlencode($link).'&title='.urlencode(fix_t($tit)).'&image=&sez=1&ep=&ep_tit=&year=';
...
foreach($videos as $video) {
  ...
  if ($title <> "..") {
    if (preg_match("/S(\d+)/",$title,$m))
     ...
     $sez="1";
    $link_f=$fs_target.'?tip=series&link='.urlencode($link).'&title='.urlencode(fix_t($tit)).'&image='.$image."&sez=".$sez."&ep=&ep_tit=&year=".$year;
  if ($n==0) echo '<TR>'."\r\n";
