...
$file=$base_fav."gdriveplayer_s.dat";
?>
<html><head>
...
foreach($arr as $key => $value) {
  ...
  if (preg_match("/\((\d+)\)/",$rest,$m)) {
   ...
   $tit_imdb=$tit_serial;
    ...
    $link_f=$fs_target.'?tip=series&link='.urlencode($link).'&title='.urlencode(fix_t($title)).'&image='.$image."&sez=".$sez."&ep=&ep_tit=&year=".$year."&imdb=";
  if ($n==0) echo '<TR>'."\r\n";
