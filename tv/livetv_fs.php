...
'Referer: http://livetv.sx/');
  ...
  //echo $h;
//$videos=explode("show_webplayer('",$h);
preg_match_all("/show_webplayer\(\'([^\']+).+href\=\"([^\"]+)\"/",$h,$m);
//print_r ($m);
...
preg_match_all("/onclick\=\"window\.open.+href\=\"([^\"]+)\"/",$h,$m);
//print_r ($m[1]);
//echo count($m[1])."\n";
  for ($k=0;$k<count($m[1]);$k++) {
  //echo "\n".fixurl($m[1][$k])."\n";
  if (preg_match("/webplayer\d?\.php/i",$m[1][$k])) {
   ...
   $h1=$t1[1];
   if (!$h1) {
   $t1=explode('id="playerblock"',$h);
   $h1=$t1[1];
   }
   if (preg_match("/\<iframe.+src\=\"([^\"]+)\"/",$h1,$n)) {
   $l=fixurl($n[1]);
   //$r[]=fixurl($m[1][$k]);
   //$s[]="ifr";
   $r[]=$l;
