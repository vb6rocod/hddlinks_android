...
<?php
include ("../common.php");
$search= unfix_t(urldecode($_GET["link"]));
...
$cookie=$base_cookie."tvmobil.dat";
  ...
  $html = curl_exec($ch);
  curl_close($ch);
  $t1=explode('Prive&#351;te pe telefon',$html);
  $t2=explode('Prive&#351;te pe laptop',$t1[1]);
  $html=$t2[0];
...
foreach($videos as $video) {
    ...
    if (preg_match('/((sop\:)[\.\d\w\-\.\/\\\:\?\&\#\%\_\,]*)/', $h1, $m)) {
      ...
      echo '<TD style="text-align:center">'.'<a href="'.$link3.'" target="_blank">'.$title.'</a></TD>';
      ...
      echo '<TD style="text-align:center">'.'<a href="'.$link2.'" target="_blank">'.$title.'</a></TD>';
      else
      echo '<TD style="text-align:center">'.'<a href="'.$link.'" target="_blank">'.$title.'</a></TD>';
    ...
    $l="link=".urlencode(fix_t($file))."&title=".urlencode(fix_t($page_title))."&from=".$from."&mod=".$mod;
    if ($flash != "mp")
    echo '<TD style="text-align:center">'.'<a href="'.$link.'" target="_blank">'.$title.'</a></TD>';
    else
    echo '<TD style="text-align:center">'.'<a onclick="ajaxrequest('."'".$l."')".'"'." style='cursor:pointer;'>".$title.'</a></TD>';
   }
}
echo "</TR></table>";
?>
