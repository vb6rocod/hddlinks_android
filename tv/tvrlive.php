...
$link="https://www.tvrplus.ro/";
  ...
  $html = curl_exec($ch);
  curl_close($ch);
$videos = explode('div class="big-gallery', $html);
unset($videos[0]);
...
foreach($videos as $video) {
    ...
    $t2=explode('"',$t1[1]);
    $title=trim($t2[0]);
    $link1="direct_link.php?link=".$link."&title=".urlencode($title)."&from=tvrlive&mod=direct";
    $l="link=".urlencode(fix_t($link))."&title=".urlencode(fix_t($title))."&from=tvrlive&mod=direct";
  if ($link <> "") {
  if ($n==0) echo '<TR>';
  if ($flash == "flash")
