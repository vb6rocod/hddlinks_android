...
$l="http://hd4all.ml/d//channels.json";
    $ch = curl_init();
    ...
    $html = curl_exec($ch);
    curl_close($ch);
    $html=str_replace('"ids"-','',$html);
    //echo $html;
    $r=json_decode($html,1);
    //print_r ($r);
    //{"ids"-"id":2933,"name":"Agro TV",
...
$n=0;
  ...
  $image=$r[$k]['icon'];
    ...
    $link1="direct_link.php?link=".$link."&title=".urlencode($title)."&from=flc1&mod=direct";
    $l="link=".urlencode(fix_t($link))."&title=".urlencode(fix_t($title))."&from=flc1&mod=direct";
  if ($link <> "") {
  if ($n==0) echo '<TR>';
  if ($flash != "mp") {
