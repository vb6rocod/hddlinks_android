...
$videos = array_values($videos);
  ...
  $t1=explode('href="/live',$video);
  $t2=explode('"',$t1[1]);
  $link="http://tvhd-online.com/live".$t2[0];
  ...
  $image=$t2[0];
    ...
    $link1="direct_link.php?link=".$link."&title=".urlencode($title)."&from=tvhd&mod=direct";
    $l="link=".urlencode(fix_t($link))."&title=".urlencode(fix_t($title))."&from=tvhd&mod=direct";
  if ($link <> "") {
  if ($n==0) echo '<TR>';
  if ($flash != "mp") {
