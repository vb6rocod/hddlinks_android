...
file_put_contents($base_cookie."adult.dat",urldecode($search1));
 ...
 //https://zbporn.com/search/2/?q=mom+son
 $search3 = "https://zbporn.com/search/".$page1."/?q=".$search1;
}
?>
<html><head>
...
if ($page1) {
if ($page1 > 1)
echo '<a href="zbporn.php?page1='.($page1-1).'&src='.$search1.'">&nbsp;&lt;&lt;&nbsp;</a> | <a href="zbporn.php?page1='.($page1+1).'&src='.$search1.'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
else
echo '<a href="zbporn.php?page1='.($page1+1).'&src='.$search1.'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
//echo $search3;
...
if ($page > 1)
echo '<a href="zbporn.php?page='.($page-1).','.$search.','.urlencode($page_title).'">&nbsp;&lt;&lt;&nbsp;</a> | <a href="zbporn.php?page='.($page+1).','.$search.','.urlencode($page_title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
else
echo '<a href="zbporn.php?page='.($page+1).','.$search.','.urlencode($page_title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
...
$search3  = $search.$page."/";  //http://www.4tube.com/videos?p=2&sort=date
  ...
  curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US; rv:1.9.1.2) Gecko/20090729 Firefox/3.5.2 GTB5');
  curl_setopt($ch, CURLOPT_FOLLOWLOCATION  ,1);
  curl_setopt($ch, CURLOPT_REFERER, "https://www.porn.com/");
  curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
  curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 5);
  curl_setopt($ch, CURLOPT_TIMEOUT, 15);
  $html = curl_exec($ch);
...
foreach($videos as $video) {
    ...
    $t1=explode('class="th-time"',$video);
    $t2=explode('>',$t1[1]);
    $t3=explode("<",$t2[1]);
    $data=" (".$t3[0].")";
  if ($n==0) echo '<TR>';
  if ($flash != "mp") {
  $link = "zbporn_link.php?file=".$link."&title=".urlencode($title);
