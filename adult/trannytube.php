...
if($tip=="release") {
  ...
  if ($page > 1)
    $l="https://www.trannytube.net/search/".$search."?page=".$page;
  else
    $l="https://www.trannytube.net/search/".$search."?page=".$page;
}
...
foreach($videos as $video) {
  ...
  $t4=explode('<div class="tube-card-title"',$video);
  $t1=explode('>',$t4[1]);
  $t3=explode('<',$t1[1]);
  $title=$t3[0];
  ...
  $t2 = explode('"', $t1[1]);
  $image = $t2[0];
  $t4=explode('<div class="tube-card-duration">',$video);
  $t1=explode("<span>",$t4[1]);
  $t3=explode("<",$t1[1]);
  $durata=trim($t3[0]);
  $durata = preg_replace("/\n|\r/"," ",strip_tags($durata));
