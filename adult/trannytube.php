...
file_put_contents($base_cookie."adult.dat",urldecode($search1));
 ...
 $search3 = "https://www.trannytube.net/search/".$search1."?page=".$page1;
}
?>
<html><head>
...
function ajaxrequest(title, link) {
  ...
  var php_file="trannytube_link.php";
...
if ($page1) {
if ($page1 > 1)
echo '<a href="trannytube.php?page1='.($page1-1).'&src='.$search1.'">&nbsp;&lt;&lt;&nbsp;</a> | <a href="trannytube.php?page1='.($page1+1).'&src='.$search1.'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
else
echo '<a href="trannytube.php?page1='.($page1+1).'&src='.$search1.'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
//echo $search3;
...
if ($page > 1)
echo '<a href="trannytube.php?page='.($page-1).','.$search.','.urlencode($page_title).'">&nbsp;&lt;&lt;&nbsp;</a> | <a href="trannytube.php?page='.($page+1).','.$search.','.urlencode($page_title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
else
echo '<a href="trannytube.php?page='.($page+1).','.$search.','.urlencode($page_title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
...
$search3  = $search."?page=".$page;  //http://www.4tube.com/videos?p=2&sort=date
  ...
  curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US; rv:1.9.1.2) Gecko/20090729 Firefox/3.5.2 GTB5');
  curl_setopt($ch, CURLOPT_FOLLOWLOCATION  ,1);
  curl_setopt($ch, CURLOPT_REFERER, "https://www.trannytube.net/");
  curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
  curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 5);
  curl_setopt($ch, CURLOPT_TIMEOUT, 15);
  $html = curl_exec($ch);
...
foreach($videos as $video) {
    ...
    $t2 = explode('"', $t1[1]);
    $link = "https://www.trannytube.net".$t2[0];
  ...
  if ($n==0) echo '<TR>';
  if ($flash != "mp") {
  $link = "trannytube_link.php?file=".$link."&title=".urlencode($title);
