...
if($tip=="release") {
 if (strpos($link,"latest-updates") !== false)
  ...
  if ($page > 1)
    $l="https://mangovideo.pw/search/".str_replace(" ","-",$tit)."/?mode=async&function=get_block&block_id=list_videos_videos_list_search_result&q=".str_replace(" ","+",$tit)."&category_ids=&sort_by=&from_videos=".$page."&from_albums=".$page."&_=";
  else
    $l="https://mangovideo.pw/search/".str_replace(" ","-",$tit)."/?mode=async&function=get_block&block_id=list_videos_videos_list_search_result&q=".str_replace(" ","+",$tit)."&category_ids=&sort_by=&from_videos=".$page."&from_albums=".$page."&_=";
}
