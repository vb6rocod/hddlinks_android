...
file_put_contents($base_cookie."adult.dat",urldecode($search1));
 ...
 $search2="http://www.xnxx.com/?k=".$search3."&p=".$page1;
 //http://www.xnxx.com/?k=mom+son&p=3
 if ($page1 > 1)
   $search2= "https://xhamster.com/search.php?q=".$search3."&qcat=video&page=".$page1;
 else
   $search2= "https://xhamster.com/search.php?q=".$search3."&qcat=video";
...
if (!$page1) {
if ($page > 1)
echo '<a href="xhamster.php?page='.($page-1).','.$search.','.urlencode($page_title).'">&nbsp;&lt;&lt;&nbsp;</a> | <a href="xhamster.php?page='.($page+1).','.$search.','.urlencode($page_title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
else
echo '<a href="xhamster.php?page='.($page+1).','.$search.','.urlencode($page_title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
} else {
if ($page1 > 1)
echo '<a href="xhamster.php?page1='.($page1-1).'&src='.$search1.'">&nbsp;&lt;&lt;&nbsp;</a> | <a href="xhamster.php?page1='.($page1+1).'&src='.$search1.'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
else
echo '<a href="xhamster.php?page1='.($page1+1).'&src='.$search1.'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
}
//$html = file_get_contents($search.$page.".html");
if (!$page1) {
  if (preg_match("/-(\d+)\.html/",$search)) {
    $r="-".$page.".html";
    $search=preg_replace("/-(\d+)\.html/",$r,$search);
  } else {
    if ($page>1) $search=$search."/".$page;
  }
  $ch = curl_init();
...
foreach($videos as $video) {
    ...
    $title1=str_between($video,'alt="','"');
    $durata=str_between($video,'duration">','<');
    $title=$title1." (".$durata.")";
