...
if($tip=="release") {
  if ($page>1)
    $l="https://portalultautv.net/filme-erotice-online/page/".$page."/";
  else
    $l="https://portalultautv.net/filme-erotice-online/";
} else {
  $search=str_replace(" ","+",$tit);
  if ($page > 1)
    $l="https://portalultautv.net/page/".$page."/?s=".$search;
  else
    $l="https://portalultautv.net/?s=".$search;
}
...
foreach($videos as $video) {
  ...
  if ($title && preg_match("/Erotice/i",$cat)) array_push($r ,array($title,$link, $image));
}
$c=count($r);
for ($k=0;$k<$c;$k++) {
  $title=$r[$k][0];
