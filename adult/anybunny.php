...
file_put_contents($base_cookie."adult.dat",urldecode($search1));
 ...
 if ($page1 > 1)
 $search="http://anybunny.com/new/".$search1."?p=".$page1;
 else
 $search="http://anybunny.com/new/".$search1;
...
function ajaxrequest(title, link) {
  ...
  var php_file="anybunny_link.php";
...
if ($page1) {
if ($page1 > 1)
echo '<a href="anybunny.php?page1='.($page1-1).'&src='.$search1.'">&nbsp;&lt;&lt;&nbsp;</a> | <a href="anybunny.php?page1='.($page1+1).'&src='.$search1.'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
else
echo '<a href="anybunny.php?page1='.($page1+1).'&src='.$search1.'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
...
if ($page > 1)
echo '<a href="anybunny.php?page='.($page-1).','.$search.','.urlencode($page_title).'">&nbsp;&lt;&lt;&nbsp;</a> | <a href="anybunny.php?page='.($page+1).','.$search.','.urlencode($page_title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
else
echo '<a href="anybunny.php?page='.($page+1).','.$search.','.urlencode($page_title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
//$html = file_get_contents($search."?p=".$page);
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL, $search."?p=".$page);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
  curl_setopt($ch,CURLOPT_REFERER,$search."?p=".$page);
  curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US; rv:1.9.1.2) Gecko/20090729 Firefox/3.5.2 GTB5');
  curl_setopt($ch, CURLOPT_FOLLOWLOCATION  ,1);
  curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
  curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 5);
...
foreach($videos as $video) {
    ...
    //$title=str_between($video,"class='clip'>",'<');
  if ($title) {
  if ($n==0) echo '<TR>';
  if ($flash != "mp") {
  $link = "anybunny_link.php?file=".$link."&title=".urlencode($title);
