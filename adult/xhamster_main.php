...
echo '<TR><td style="color:black;background-color:#0a6996;color:#64c8ff;text-align:center" colspan="3"><font size="6"><b>xhamster</b></TD></TR>';
echo '<TR><TD class="cat"><a href="">Reload..</a></TD>';
...
echo '<TD class="form" colspan="2"><form action="xhamster.php" target="_blank">Cautare <input type="hidden" name="page1" id="page1" value="1">
<input type="text" id="src" name="src" value="'.$val_search.'">
<input type="submit" value="Cauta" id="send"></form></td>';
...
foreach($videos as $video) {
    ...
    //$link=substr($link, 0, -6);
  	...
  	$t4=explode("<",$t3[1]);
  	$title=trim($t4[0]);
    $link="xhamster.php?page=1,".$link.",".urlencode($title);
    if ($title) {
	if ($n == 0) echo "<TR>"."\n\r";
