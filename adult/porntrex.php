...
if($tip=="release") {
 if ($link=="latest-updates")
  $l="https://www.porntrex.com/latest-updates/".$page."/?mode=async&function=get_block&block_id=list_videos_latest_videos_list_norm&sort_by=post_date&from=".$page."&_=1556827037437";
 else
  $l="https://www.porntrex.com/categories/".$link."?mode=async&function=get_block&block_id=list_videos_common_videos_list_norm&sort_by=post_date&from4=".$page."&_=1556827472744";
} else {
  $search=str_replace(" ","-",$tit);
  if ($page > 1)
    $l="https://www.porntrex.com/search/".$search."/?mode=async&function=get_block&block_id=list_videos_videos&q=".$search."&category_ids=&sort_by=latest-updates&from_videos=".$page."&from_albums=02&_=1556827208252";
  else
    $l="https://www.porntrex.com/search/".$search."/?mode=async&function=get_block&block_id=list_videos_videos&q=".$search."&category_ids=&sort_by=latest-updates&from_videos=".$page."&from_albums=02&_=1556827208252";
}
...
foreach($videos as $video) {
  ...
  if ($title && $priv != "Private") array_push($r ,array($title,$link, $image));
}
$c=count($r);
for ($k=0;$k<$c;$k++) {
  $title=$r[$k][0];
