...
function ajaxrequest(title, link) {
  ...
  var php_file="porntrex_link.php";
...
if ($page1) {
if ($page1 > 1)
echo '<a href="porntrex.php?page1='.($page1-1).'&src='.$search1.'">&nbsp;&lt;&lt;&nbsp;</a> | <a href="porntrex.php?page1='.($page1+1).'&src='.$search1.'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
else
echo '<a href="porntrex.php?page1='.($page1+1).'&src='.$search1.'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
//echo $search3;
...
//https://www.youjizz.com/search/mature-mom-anal-1.html?
  ...
  curl_setopt($ch, CURLOPT_REFERER, "https://www.porntrex.com");
...
if ($page > 1)
echo '<a href="porntrex.php?page='.($page-1).','.$search.','.urlencode($page_title).'">&nbsp;&lt;&lt;&nbsp;</a> | <a href="porntrex.php?page='.($page+1).','.$search.','.urlencode($page_title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
else
echo '<a href="porntrex.php?page='.($page+1).','.$search.','.urlencode($page_title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
//$html = file_get_contents($search."?page=".$page);
//https://www.porn.com/videos?p=2
if ($search=="latest-updates")
 $search3="https://www.porntrex.com/latest-updates/".$page."/?mode=async&function=get_block&block_id=list_videos_latest_videos_list_norm&sort_by=post_date&from=".$page."&_=1556827037437";
else
 $search3="https://www.porntrex.com/categories/".$search."?mode=async&function=get_block&block_id=list_videos_common_videos_list_norm&sort_by=post_date&from4=".$page."&_=1556827472744";
...
foreach($videos as $video) {
    ...
    $t2 = explode('?', $t1[1]);
    $image = "https:".$t2[0];
    $t1=explode('span class="ico-private">',$video);
    $t2=explode('<',$t1[1]);
    $priv=$t2[0];
    //$image=str_replace("1.jpg","2.jpg",$image);
    //$image = str_replace("https","http",$image);
    $image="../filme/r_m.php?file=".$image;
    $t1=explode('div class="durations">',$video);
    //$t2=explode('>',$t1[1]);
    $t3=explode("</div",$t1[1]);
    $data=" (".trim(strip_tags($t3[0])).")";
    if ($priv != "Private") {
  if ($n==0) echo '<TR>';
  if ($flash != "mp") {
  $link = "porntrex_link.php?file=".$link."&title=".urlencode($title);
