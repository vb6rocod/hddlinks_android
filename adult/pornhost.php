...
file_put_contents($base_cookie."adult.dat",urldecode($search1));
 ...
 //https://www.pornhost.com/search.html?search=mom%20son&type=0&favorite=all&uploader=&sort=gallery_date&start=1
 $search3 = "https://www.pornhost.com/search.html?search=".str_replace("+","%20",$search1)."&type=0&favorite=all&uploader=&sort=gallery_date&start=".($page1-1);
}
?>
<html><head>
...
function ajaxrequest(title, link) {
  ...
  var php_file="pornhost_link.php";
...
if ($page1) {
if ($page1 > 1)
echo '<a href="pornhost.php?page1='.($page1-1).'&src='.$search1.'">&nbsp;&lt;&lt;&nbsp;</a> | <a href="pornhost.php?page1='.($page1+1).'&src='.$search1.'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
else
echo '<a href="pornhost.php?page1='.($page1+1).'&src='.$search1.'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
//echo $search3;
...
//https://www.youjizz.com/search/mature-mom-anal-1.html?
  ...
  curl_setopt($ch, CURLOPT_REFERER, "https://www.pornhost.com/");
...
if ($page > 1)
echo '<a href="pornhost.php?page='.($page-1).','.$search.','.urlencode($page_title).'">&nbsp;&lt;&lt;&nbsp;</a> | <a href="pornhost.php?page='.($page+1).','.$search.','.urlencode($page_title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
else
echo '<a href="pornhost.php?page='.($page+1).','.$search.','.urlencode($page_title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
//$html = file_get_contents($search."?page=".$page);
//https://www.porn.com/videos?p=2

$search3  = $search."?pagenr=".$page;  //http://www.4tube.com/videos?p=2&sort=date
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL, $search3);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
  curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US; rv:1.9.1.2) Gecko/20090729 Firefox/3.5.2 GTB5');
...
$videos = explode('class="videos">',$html);
...
foreach($videos as $video) {
    ...
    $t2 = explode('"', $t1[1]);
    $link = "https://www.pornhost.com".$t2[0];
    ...
    $t1=explode('class="rating"',$video);
    $t2=explode('>',$t1[1]);
    $t3=explode("<",$t2[1]);
    $data=" (".$t3[0].")";
  if ($n==0) echo '<TR>';
  if ($flash != "mp") {
  $link = "pornhost_link.php?file=".$link."&title=".urlencode($title);
