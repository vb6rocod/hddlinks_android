...
if($tip=="release") {
  ...
  if ($page > 1)
    $l="https://www.proporn.com/search/videos/".$search."/".$page;
  else
    $l="https://www.proporn.com/search/videos/".$search."/".$page;
}
...
$r=array();
$videos = explode('<a class="thumb-link"',$html);
unset($videos[0]);
...
foreach($videos as $video) {
  ...
  $t2 = explode('"', $t1[1]);
  $image = $t2[0];
  $t1=explode('class="duration-badge">',$video);
  //$t2=explode('>',$t1[1]);
  $t3=explode("<",$t1[1]);
  $durata=trim($t3[0]);
  $durata = preg_replace("/\n|\r/"," ",strip_tags($durata));
