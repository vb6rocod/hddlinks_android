...
if($tip=="release") {
  if ($page==1)
  $l="https://www.xfreehd.com/videos";
  else
  $l="https://www.xfreehd.com/videos?page=".$page;
} else {
  $search=str_replace(" ","+",$tit);
  $l="https://www.xfreehd.com/search?search_query=".$search."&search_type=videos&page=".$page;
}
$host=parse_url($l)['host'];
  ...
  curl_setopt($ch, CURLOPT_REFERER,"https://www.xfreehd.com");
...
//echo $html;
$r=array();
$videos = explode('a class="video-link',$html);
unset($videos[0]);
...
foreach($videos as $video) {
  ...
  $t2 = explode('"', $t1[1]);
  $image = $t2[0];
  if (!$image) $image="../filme/blank.jpg";
  $t1=explode('class="duration-new">',$video);
  $t2=explode('<',$t1[1]);
  $durata = trim($t2[0]);
