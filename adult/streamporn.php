...
if ($page1) {
if ($page1 > 1)
echo '<a href="streamporn.php?page1='.($page1-1).'&src='.$search1.'">&nbsp;&lt;&lt;&nbsp;</a> | <a href="streamporn.php?page1='.($page1+1).'&src='.$search1.'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
else
echo '<a href="streamporn.php?page1='.($page1+1).'&src='.$search1.'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
//$post = "s=".$search;
//
if ($page1>1)
  $search3="https://streamporn.pw/page/".$page1."?s=".$search;
else
  $search3="https://streamporn.pw/?s=".$search;
...
if ($page > 1)
echo '<a href="streamporn.php?page='.($page-1).','.$search.','.urlencode($page_title).'">&nbsp;&lt;&lt;&nbsp;</a> | <a href="streamporn.php?page='.($page+1).','.$search.','.urlencode($page_title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
else
echo '<a href="streamporn.php?page='.($page+1).','.$search.','.urlencode($page_title).'">&nbsp;&gt;&gt;&nbsp;</a></TD></TR>';
  //https://streamporn.pw/movies/page/2
...
foreach($videos as $video) {
    ...
    //$t2=explode(">",$t1[1]);
    //$t3=explode("<",$t2[1]);
    $t1=explode('qtip-title">',$video);
    $t2=explode('<',$t1[1]);
    $title=strip_tags($t2[0]);
    $link = "../filme/filme_link.php?file=".urlencode($link)."&title=".urlencode($title);
  if ($title) {
  if ($n==0) echo '<TR>';
  echo '<td class="mp" align="center" width="25%"><a href="'.$link.'" target="_blank"><img src="'.$image.'" width="200px" height="280px"><BR>'.$title.'</a></TD>';
  $n++;
  if ($n == 4) {
  echo '</tr>';
  $n=0;
